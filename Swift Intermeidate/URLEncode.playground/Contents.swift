...
func encodeParametersInUrl(_ parameters: [String: Any]) -> String {
    ...
    for key in sortedKeys {
        ...
        let queryComponents = queryComponetsWith(key: key, value: value)
        components.append(contentsOf: queryComponents)
        
    }
    let encodedComponents = components.map {
        "\($0)=\($1)"
    }
...
func queryComponetsWith(key: String, value: Any) -> [(String, String)] {
    ...
    if let dictionary = value as? [String: Any]{
        for (nestedKey, value) in dictionary {
            let nestedComponents = queryComponetsWith(key: "\(key)[\(nestedKey)]", value: value)
            components.append(contentsOf: nestedComponents)
        }
    } else if let array = value as? [Any] {
        for value in array {
            let nestedComponents = queryComponetsWith(key: "\(key)[]", value: value)
            components.append(contentsOf: nestedComponents)
        }
    } else {
